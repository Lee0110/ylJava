...
### 建造者模式在com.google.common.cache.CacheBuilder类中的应用
...
为什么这里一定要用建造者模式，而不是无参构造一个，再调用set方法呢？从源码可以看出，使用建造者模式可以校验参数，直接new出来的对象可能不合法或者不可用。

### Wrapper模式在com.google.common.collect包里的应用

代理模式、装饰器、适配器模式可以统称为 Wrapper 模式，通过Wrapper类二次封装原始类。它们的代码实现也很相似，都可以通过组合的方式，将Wrapper类的函数实现委托给原始类的函数来实现。

一组以 Forwarding 开头命名的类。他们的实现方式类似，截取部分代码如下
...
```java
com.google.common.collectpublic class AddLoggingCollection<E> extends ForwardingCollection<E> {
    private static final Logger logger = LoggerFactory.getLogger(ForwardingCollection.class);
...
### Immutable 模式在 com.google.common.collect中的应用

JDK 不变集合和 Google Guava 不变集合都不可增删数据。但是，当原始集合增加数据之后，JDK 不变集合的数据随之增加，而 Google Guava 的不变集合的数据并没有增加。
...
其中，ApplicationEvent 和 ApplicationListener 的代码实现都非常简单，内部并不包含太多属性和方法。实际上，它们最大的作用是做类型标识之用（继承自 ApplicationEvent的类是事件，实现 ApplicationListener 的类是监听器）
